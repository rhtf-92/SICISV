...
export function NavigationTabs({ activeTab, onTabChange, pendingExits = 0, role = 'guard' }: NavigationTabsProps) {
  const allTabs: Tab[] = [
  ...
  // Filter tabs according to strict operational matrix
  const tabs = allTabs.filter(tab => {
    if (role === 'guard') {
      return tab.id === 'entry' || tab.id === 'exit';
    }
    if (role === 'support') {
